...
class JsonSerializerGenerator(
    ...
    private val coreJsonSerializer = CoreJsonSerializer(
        symbolProvider,
        model,
        runtimeConfig,
        jsonName,
        ...
        return RuntimeType.forInlineFun(fnName, operationSerModule) {
            it.rustBlockTemplate(
                ...
                *codegenScope,
                ...
                with(coreJsonSerializer) {
                    serializeStructure(
                        CoreJsonSerializer.StructContext(
                            "object",
                            "value",
                            structureShape
                        ),
                ...
                *codegenScope,
                ...
                with(coreJsonSerializer) {
                    when (target) {
                        ...
                        is UnionShape -> serializeUnion(
                            CoreJsonSerializer.Context(
                                "object",
                                ValueExpression.Reference("input"),
                                target
                            )
                        )
...
class CoreJsonSerializer(
    ...
    private fun RustWriter.serializeMemberValue(context: MemberContext, target: Shape) {
        ...
        when (target) {
            ...
            is TimestampShape -> {
                val timestampFormat = timestampFormat(context.shape)
